...
//-----------------------------------------------------------------------------
long _EERIEMouseXdep, _EERIEMouseYdep = 0;
...
HRESULT CSdlApplication::Initialize3DEnvironment()
{
	...
	dwFrameworkFlags |= (!m_pDeviceInfo->bWindowed ? FW_FULLSCREEN : 0L);
	dwFrameworkFlags |= (m_pDeviceInfo->bStereo   ? FW_STEREO     : 0L);
	dwFrameworkFlags |= (m_bAppUseZBuffer         ? FW_ZBUFFER    : 0L);
...
HRESULT CSdlApplication::SetClipping(float x1, float y1, float x2, float y2)
{
	...
	if (FAILED(m_pd3dDevice->SetViewport(&vp))) return FWERR_NOVIEWPORT;
...

{
	int i;
	g_pApp->Pause(TRUE);
	i = MessageBox(g_pApp->m_hWnd, text, title, MB_ICONQUESTION | MB_OKCANCEL);
	g_pApp->Pause(FALSE);
...
void ShowPopup(char * text)
{
	...
	g_pApp->Pause(TRUE);
	MessageBox(g_pApp->m_hWnd, text, "GAIA popup", MB_ICONINFORMATION | MB_OK);
	g_pApp->Pause(FALSE);
}
...
int ShowError(char * funcname, char * message, long fatality)
{
	...
	switch (fatality)
	{
		case 0:
			...
			re = MessageBox(g_pApp->m_hWnd, texx, "GAIA Error Message", MB_ICONERROR | MB_OKCANCEL);
			g_pApp->Pause(FALSE);
		...
		case 1:
			...
			re = MessageBox(g_pApp->m_hWnd, texx, "GAIA Error Message", MB_ICONSTOP | MB_ABORTRETRYIGNORE);
			g_pApp->Pause(FALSE);
		...
		case 2:
			...
			sprintf(texx, "An ERROR occured in function: %s\n%s\nFatality Level: %d - %s", funcname, message, fatality, fatall);
			re = (MessageBox(g_pApp->m_hWnd, texx, "GAIA Error Message", MB_ICONERROR | MB_OK));
			g_pApp->Pause(FALSE);
			return re;
			break;
		case 3:
